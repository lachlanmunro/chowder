...
func main() {
	...
	pretty := flag.Bool("pretty", true, "Use pretty logging (instead of JSON)")
	...
	integerDurations := flag.Bool("intdur", true, "Log integer durations instead of floats")
	flag.Parse()
	// Setup the logger
	if *pretty {
	...
	zerolog.DurationFieldInteger = *integerDurations
	// Get the auth list
